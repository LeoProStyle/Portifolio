                ...
                <tr key={item.id} className="border-b border-zinc-100 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-950/50">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-2">
                      ...
                      <a href={`/despesas/novo?edit=${item.id}`} className="text-sm text-blue-600">Editar</a>
                      <button onClick={async () => {
                        const r = await Swal.fire({
                          title: 'Confirma excluir esta despesa?',
                          icon: 'warning',
                        ...
                        try {
                          const res = await fetch('/api/expenses', { method: 'DELETE', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ id: item.id }) });
                          const json = await res.json();
