...
export default function PurchaseNoteForm({ defaultDate, onCreated, noteId }: { defaultDate: string; onCreated?: () => void; noteId?: string; }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      Swal.close();
      Swal.fire({ icon: 'success', title: 'Nota salva!', confirmButtonText: 'OK' }).then(() => {
        onCreated?.();
        router.push('/notas-de-compras');
      });
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Erro inesperado';
      setError(message);
    ...
    <form onSubmit={submit} className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="space-y-2">
          ...
          <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full rounded-xl border border-zinc-200 bg-white px-3 py-2 text-sm dark:border-zinc-800 dark:bg-zinc-950" />
        </div>
        <div className="space-y-2">
          <label className="text-sm font-medium">Categoria</label>
          <select value={category} onChange={(e) => setCategory(e.target.value as ExpenseCategory)} className="w-full rounded-xl border border-zinc-200 bg-white px-3 py-2 text-sm dark:border-zinc-800 dark:bg-zinc-950">
            {( [ 'Mercadoria','Bebidas','Embalagens','Gás','Energia','Água','Internet','Aluguel','Marketing','Limpeza','Manutenção','Impostos','Outros' ] as ExpenseCategory[] ).map((c) => (<option key={c} value={c}>{c}</option>))}
          </select>
        </div>
      </div>
      ...
      <div className="rounded-2xl border bg-white dark:bg-zinc-900 p-4">
        <div className="space-y-2">
          ...
          <input inputMode="decimal" type="number" step="0.01" value={amount} onInput={(e) => { const el = e.currentTarget; const n = toNumber(el.value); if (el.value !== String(n)) el.value = String(n); setAmount(n); }} className="w-full rounded-xl border border-zinc-200 bg-white px-3 py-2 text-sm dark:border-zinc-800 dark:bg-zinc-950" />
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="flex items-center gap-3">
          <input id="hasFiscal2" type="checkbox" checked={hasFiscalDocument} onChange={(e) => setHasFiscalDocument(e.target.checked)} />
          <label htmlFor="hasFiscal2" className="text-sm">Possui documento fiscal</label>
        </div>
        <div className="space-y-2">
      ...
      <div className="flex items-center gap-3">
        <input id="active2" type="checkbox" checked={active} onChange={(e) => setActive(e.target.checked)} />
        <label htmlFor="active2" className="text-sm">Ativo</label>
      </div>
      ...
      <button type="submit" disabled={!date || !category || !description || typeof amount !== 'number' || loading} className="w-full rounded-xl bg-zinc-900 text-white px-4 py-2.5 text-sm font-medium disabled:opacity-60 dark:bg-white dark:text-black">{loading ? 'Salvando...' : noteId ? 'Atualizar nota' : 'Salvar nota'}</button>
    </form>
  );
}
